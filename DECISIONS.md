...
## D36. The all-data repeat gives the same answer, and pat09's rationale is restated

**Date:** 2026-09-15. Full table: `outputs/results_alldata/COHORT_RESULT.md`,
regenerated by `scripts/combine_cohorts.py`.

Stage 7 was repeated end to end on the 14-patient backbones (D35 rule 3): signatures
rebuilt on the new frozen features, eight hypernetwork runs, the shuffled-z control.
Nothing else changed. The two experiments differ in exactly one variable -- 5 training
patients per fold against 11.

**Across both experiments, 0 of 14 defined z-scores pass D28's z > 2.0.** The range is
-2.01 to +0.93, and the only |z| > 2 anywhere is pat02 at **-2.01** on the 14-patient
backbone: the patient's own signature significantly *worse* than a stranger's. On the
D35 analysis set the personalisation estimate moved from -0.00048 (exact p = 0.0625) to
-0.00068 (exact p = 0.2500) -- slightly further negative, and less consistent between
folds, not more.

**The "it just needed more data" explanation is closed.** It was the single most
plausible defence of the negative result and the reason the repeat was approved.

**§3.5's correlations flip sign between cohorts**: corr(D_p, own - baseline) goes
+0.049 -> -0.134 and corr(D_p, own - shuffled) goes -0.566 -> +0.048 (all folds); on the
D35 set, -0.495 -> -0.155 and -0.706 -> +0.185. A sign that reverses under a change of
training-set size is not a small effect measured imprecisely. These are AUC-based and so
secondary under D35; the primary FDR/h version at n = 7 is still outstanding.

**Scale of the nuisance variation.** Across the D35 folds the mean baseline is
essentially unchanged (0.642285 -> 0.642298) while individual folds move by up to 0.415
AUC in both directions (pat07 0.967 -> 0.552; pat09 0.180 -> 0.567). Adding six training
patients reshuffles which held-out patients a backbone happens to suit without changing
how well it does on average. Any personalisation effect has to be visible against that.

### D35 rule 1 restated, not inherited

Rule 1 excludes pat09 from §3.5's primary analysis because its Phase 1 model suffered
feature collapse -- held-out within-source AUC 0.180, anti-correlated with the truth.
**That collapse does not recur on the 14-patient backbone, where the same fold scores
0.567.** The rationale is specific to Phase 1 and the paper must say so; for the
14-patient cohort pat09 is an ordinary fold and its exclusion is reported for continuity
...
- ~~Score the eight new backbones on the same held-out test clips.~~ DONE --
  `outputs/results_alldata/ALLDATA_BASELINE.md`; the mean moved by +0.0001.
- Re-select per-fold decision thresholds (D16's procedure) for the new backbones —
  required, because the FDR/h analysis below is to be reported for both cohorts.
