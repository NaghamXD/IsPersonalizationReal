...
# --- CONFIGURATION ---
JSON_FILE = "evaluation_report_improved.json"
OUTPUT_IMAGE = "clinical_performance_dashboard_improved.png"
...
def generate_figure():
    ...
    # LEO = Latency from EEG Onset (Positive = After, Negative = Before)
    ...
    for bar in bars:
        ...
        ax1.text(bar.get_x() + bar.get_width()/2., height + (1 if height > 0 else -3),
                 ...
                 fontsize=9, color='#004080', fontweight='bold')

    # --- 4. PLOT CLINICAL WARNING POINTS (LCO) ---
    # LCO = Latency to Clinical Onset (Negative = Early Warning)
    ax2 = ax1.twinx() # Create secondary y-axis sharing x-axis
    
    # We use a line plot with markers to contrast with bars
    ax2.plot(ids, lcos, color='#d9534f', marker='o', linestyle='-', linewidth=2, markersize=8, label='Time to Clinical Onset (LCO)')
    
    # Highlight the "Warning Zone" (Early Detection)
    # Any point below 0 on the red line is a successful early warning
    ...
    # Title
    plt.title(f"Seizure Detection Performance (Stride: {config.get('stride')}s)", fontsize=16, fontweight='bold', pad=20)

    # Legends (Combine them)
    lines_1, labels_1 = ax1.get_legend_handles_labels()
    lines_2, labels_2 = ax2.get_legend_handles_labels()
    ax1.legend(lines_1 + lines_2, labels_1 + labels_2, loc='upper left', frameon=True, fancybox=True, shadow=True)

    # --- 6. SUMMARY STATS BOX ---
    # This places a text box with the "Report Card"
    stats_text = (
        f"📊 GLOBAL METRICS\n"
        f"───────────────────\n"
        f"Sensitivity:   {metrics.get('sensitivity', 0)*100:.1f}%\n"
        f"FDR:           {fdr_data.get('fdr_per_hour', 0):.2f} /hr\n"
        f"Avg LEO:       {metrics.get('avg_leo', 0):.2f} s\n"
        f"Avg LCO:       {metrics.get('avg_lco', 0):.2f} s\n"
        f"───────────────────\n"
        f"False Alarms:  {fdr_data.get('total_false_positives', 0)}\n"
        f"Analyzed Hrs:  {fdr_data.get('total_hours_analyzed', 0):.2f} h"
    )
    
    # Place text box in top right
    plt.text(1.15, 0.95, stats_text, transform=ax1.transAxes, fontsize=12,
             verticalalignment='top', fontfamily='monospace',
             bbox=dict(boxstyle='round,pad=0.8', facecolor='#f8f9fa', edgecolor='#333', alpha=0.9))
